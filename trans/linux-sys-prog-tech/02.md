...
我们也可以使用`setenv()`函数在 C 程序中更改环境变量并创建新的环境变量。然而，当我们这样做时，那些变量在启动程序的 shell 中是不可用的。我们运行的程序是Shell的**子进程**，因此它不能改变Shell的变量；即其**母流程**。但是从我们自己的程序中启动的任何其他程序都能够看到这些变量。我们将在本书后面更深入地讨论父进程和子进程。
