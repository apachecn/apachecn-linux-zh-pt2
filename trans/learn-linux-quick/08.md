...
`-u`选项用于设置`jerry`的 UID。我们还使用`-c`选项为用户`jerry`添加评论，最后我们使用`-s`选项为`jerry`设置默认Shell。
...
注意用户`tom`的评论栏是如何空的，因为我们在创建用户`tom`时没有添加任何评论，注意用户`tom`的 UID 是如何被系统选择的，但是我们选择了用户`jerry`的`777`。另外，请注意用户`tom`的默认Shell被系统选择为`/bin/sh`，这是`/bin/bash`的旧版本。但是，我们为用户`jerry`选择了更新的Shell`/bin/bash`。
...
我们能够切换到用户`tom`，但是正如您所看到的，Shell看起来非常不同，因为命令提示符没有显示用户名或主机名。那是因为用户`tom`的默认Shell是`/bin/sh`。您可以使用`echo $SHELL`命令显示用户的默认Shell:
...
用户`jerry`的一切看起来都更好，因为我们确实将他的默认Shell设置为`/bin/bash`。好了，现在让我们切换回`root`用户:
...
所以我们不高兴用户`tom`的默认Shell是`/bin/sh`，我们想改成`/bin/bash`。我们可以使用`usermod`命令修改用户属性。
...
请注意，您也可以使用命令选项的全名；所以可以用`--shell`代替`-s`。不管怎样，让我们看看我们是否成功更改了用户`tom`的默认Shell:
...
我们厌倦了每次创建新用户时都指定默认Shell。但幸运的是，有一个文件可以为任何新创建的用户指定默认 shell。这个惊人的文件就是`/etc/default/useradd`。
...
太棒了。现在，任何新创建的用户都将把`/bin/bash`作为默认Shell。让我们通过创建一个名为`spy`的新用户来测试它:
...
万岁！我们可以看到`bash`是用户`spy`的默认Shell。

请记住`/bin/sh`和`/bin/bash`不是您系统上仅有的两个有效Shell；还有更多！查看文件`/etc/shells`查看系统上所有有效Shell的完整列表:
