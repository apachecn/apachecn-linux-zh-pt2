...
您已经学习了如何使用`whoami`命令来识别当前登录和使用Shell的用户。这个命令非常简单，它只返回用户名，没有附加信息。
...
虽然 Mint 为您提供了一个图形工具来处理用户权限，但您也可以出于同样的目的使用Shell。最有用的命令之一是`chmod`，它允许您更改文件和目录的权限。此外，由于有了`chown`命令，您可以决定谁是特定文件或目录的所有者。
