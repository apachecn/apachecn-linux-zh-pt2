...
这类似于前面的声明，因为我们有`#!`，这表示我们要定义要使用的 shell 解释器；在这种情况下，我们使用的是伯恩再次Shell，或 Bash。这个Shell提供了比常规`sh`Shell更多的扩展；事实上，大多数较新的 Linux 发行版都是用 Bash 作为实际Shell移植的。通过在终端中发出以下命令，我们可以很容易地识别哪个 shell 正在运行:
...
太棒了。环境变量`SHELL`存储当前 shell 返回的值表明我们正在运行 bash shell。另一种识别Shell的方法如下:
...
太棒了！正在使用 bash shell。另外，我们可以使用`ps`命令显示当前Shell，如下所示:
...
太棒了！查看数据库的另一种方法是使用带有`-e`选项的`mysql`命令；这允许我们从Shell执行命令。下面的代码片段显示了我们如何列出数据库:
