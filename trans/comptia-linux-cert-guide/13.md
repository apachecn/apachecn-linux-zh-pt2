...
基于前面的输出，只要我们输入一个日期(在本例中，我们以 HH:MM 的格式输入一个时间)，它就会启动`at`实用程序，我们会看到一个`warning: commands will be executed using /bin/sh`警告；这告诉我们`at`实用程序在执行时将使用哪个Shell。在这里，我们可以键入任何希望在指定时间运行的命令。例如:
