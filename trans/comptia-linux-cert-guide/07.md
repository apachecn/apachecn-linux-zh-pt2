...
**YUM 扩展器**，简称 **yumex** ，是`yum`和`dnf`实用程序的前端。默认情况下，`yumex`没有预装 Fedora 28。这可以通过在Shell上安装`yumex`实用程序来轻松解决:
...
在 CentOS 7 上安装`yumex`的步骤与 Fedora 28 非常相似。最后，我们可以在Shell或通过图形用户界面启动`yumex`实用程序。我们将演示 shell 方法:
