...
要了解上一节创建的`vi`进程的PID，我们可以使用`pidof`和`ps`等命令。例如，在新终端中运行以下命令，以了解`vi`进程的 pid:
...
在正常情况下，完成任务后，一个进程终止并释放分配的资源。如果Shell有分叉任何子进程，那么它将等待它们先完成任务(除了后台进程)。在某些情况下，某个进程可能无法正常运行，并且它可能比预期等待或消耗资源的时间更长。在其他一些情况下，可能会发生现在不再需要进程的情况。在这种情况下，我们可以从终端杀死进程并释放资源。
