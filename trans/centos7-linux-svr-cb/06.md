...
在本食谱中，我们将学习如何提供额外的安全措施，以强化安全Shell环境。 **安全Shell** ( **SSH** )是提供远程访问您的服务器的基本工具包。到远程机器的实际距离可以忽略不计，但是 shell 环境使您能够执行维护、升级、软件包安装和文件传输；作为管理员，您还可以在安全的环境中执行任何需要执行的操作。它是一个重要的工具；作为系统的网关，本食谱的目的是向您展示如何执行一些基本的配置更改，以保护您的服务器免受不需要的访客的影响。
