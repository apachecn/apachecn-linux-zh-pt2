...
![images](img/square.jpg) **Note** This control over processes is what makes Linux so reliable. Because any user can delve into the workings of the kernel and terminate individual processes, crashed programs can be cleaned up with negligible impact on the rest of the system.
...
![images](img/square.jpg) **Caution** You should be absolutely sure that you know the correct process before killing it. If you get it wrong, you could cause other programs to stop running.
...
![images](img/square.jpg) **Caution** Make sure you're as specific as possible when using the `killall` command. Issuing a command like `killall` `bin` will kill all processes that might have the word `bin` in their name!
