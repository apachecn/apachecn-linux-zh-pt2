...
![images](images/square.jpg) **提示**如果桌面偏离中心，并且不能访问菜单来改变分辨率，右击面板上的某个地方，临时添加一个新的主菜单小程序。然后，您将能够从这里访问首选项部分。
...
![images](images/square.jpg) **注**在技术层面上，纳蒂使用的是 X.org 的最新版本。这个版本的 long 能够自动检测和自动配置显示器、显卡和鼠标，这意味着很少需要手动定制显示设置——这一直是全世界 Linux 用户的心病。
...
在 Ubuntu 的显示器设置中(系统![images](images/U001.jpg)首选项![images](images/U001.jpg)显示器)，你可以为你的显示器尝试不同的分辨率，如图[图 5-2](#fig_5_2) 所示。[表 5-1](#tab_5_1) 显示了最常见的显示器分辨率。请注意，与支持多种分辨率的老式 CRT 显示器相比，平板(LCD)屏幕通常只有一种“原生”分辨率，显示清晰。
