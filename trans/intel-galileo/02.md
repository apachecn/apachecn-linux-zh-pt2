...
各代工具链需要的步骤与“*构建英特尔 Galileo 映像”*一节中提到的步骤相同；然而，`bitbake`命令不同，必须下载额外的层。
...
将这些文件和目录复制到您的微型 SD 卡中，将其插入微型 SD 卡插槽中(参见[第 1 章](01.html))，然后打开英特尔 Galileo 系统的电源。
...
6.  将电源连接到英特尔 Galileo。
7.  在串行 shell 中，会显示一个平台列表；选择 Galileo 模型。
8.  将电阻器从地面上拆下。
