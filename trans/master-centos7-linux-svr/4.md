...
![Setting up and configuring of Postfix mail server](../images/00049.jpeg)
...
![Setting up and configuring of Postfix mail server](../images/00050.jpeg)
...
现在，我们已经很好地配置了 MariaDB 服务，并且可以使用了。我们将开始建立新的数据库，与 Postfix 一起使用。要添加新数据库，我们需要打开 MariaDB 外壳:
...
我们已经成功地设置了邮件服务器数据库表的初始配置。我们现在可以离开马里亚数据库外壳:
...
![Setting up a mail tool (Dovecot) to retrieve mails](../images/00051.jpeg)
...
远程登录将连接，我们将看到远程登录外壳，我们在里面键入以下命令:
...
现在我们将测试邮件服务。为此，我们需要首先用测试域和用户填充我们的数据库。首先，我们进入 MariaDB 数据库外壳:
...
![Setting up a mail tool (Dovecot) to retrieve mails](../images/00052.jpeg)
...
![Configuring the OpenLDAP Active Directory with Postfix](../images/00053.jpeg)
