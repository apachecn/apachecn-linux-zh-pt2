...
众所周知，SSH 是我们用来远程连接到 Linux 机器的安全Shell连接。它是系统管理员用来远程管理其基础设施的主要工具。默认情况下，它是我们在 CentOS 7 和几乎所有 Linux 发行版的基本安装中找到的基本工具之一。
...
我们将使用它来过滤邮件服务器上的所有Postfix邮件(正如我们在[第 4 章](4.html#11C3M2-f9a6cd90c19b48959347647adb332073 "Chapter 4. Mail Server with Postfix")、*带有Postfix的邮件服务器、*中关于如何启动和运行邮件服务器的描述)。
...
然后我们的服务就配置好了。接下来是Postfix部分，我们应该对它的主配置文件进行一些小的更改:
