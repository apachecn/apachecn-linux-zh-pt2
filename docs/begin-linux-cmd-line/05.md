...
另一种在你的计算机上显示块设备的方法是使用 **lsblk** 命令 T3。此命令提供了计算机上所有可用存储设备的方便概述。[清单 5-6](#FPar6) 给出了该命令输出的概述。
...
除了默认创建的子卷之外，管理员还可以手动添加新的子卷。为此，使用命令 **btrfs 子卷创建**，后跟所需子卷的路径。例如，使用命令 **btrfs 子卷创建/根 T4】为用户根的主目录创建一个子卷。**
